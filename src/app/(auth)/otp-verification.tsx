...
export default function OTPVerificationScreen() {
  ...
  useEffect(() => {
    console.log('=== OTP VERIFICATION SCREEN ACCESSED ===');
    console.log('Type parameter:', type);
    
    if (!type || type !== 'password_reset') {
      console.log('Redirecting from OTP verification to login (registration flow)');
      showSuccessToast('Success', 'Account verified! Please log in.');
      router.replace('/(auth)/login');
      return;
    }
    console.log('Allowing OTP verification for password reset');
  }, [type]);
  ...
  }, [otp, isLoading, hasAutoVerified, handleVerify]);
  ...
  const handleResendCode = async () => {
    ...
    try {
      await authService.requestPasswordReset({ email: email as string });
